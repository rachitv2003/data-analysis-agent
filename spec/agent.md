...
| `save_dataset(df, name, desc)` | Materialise a DataFrame as a registered DERIVED dataset | DataFrame, name, description | confirmation string | writes `uploads/{id}.csv` + `.parquet`; inserts a `datasets` row (origin=derived) with `derivation_code` + parents + producing run (C25); ALSO injects the frame into the run namespace as a `<name>` variable for later steps, and appends the new id to the session so later turns load it |

**Sandbox namespace** (provided to `execute_action`): `df` (first DataFrame), `df1`/`df2`/… (per-dataset, ordered by `dataset_ids`), a `<filename_stem>` alias per dataset, and the libraries `pd, np, px, go, plt, sns, scipy, stats, sklearn, sm`, plus `save_dataset`. No filesystem/network builtins beyond these are exposed. **State across steps:** the namespace is rebuilt each step, so ordinary assigned variables do NOT persist — the way to carry a result forward is `save_dataset`, which appends the saved frame to the run's `_dataframes[run_id]` so the next step's namespace exposes it as `<name>` (and `POST /ask` appends the new id to the session's `dataset_ids_json`, so a later TURN loads it too). This lets a model "save a base table, then reference it by name in a later step/turn".
