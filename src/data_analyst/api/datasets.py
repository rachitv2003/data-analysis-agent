...
def preview_dataset(dataset_id: str, rows: int = 10, session: Session = Depends(get_session)):
    ...
    try:
        ...
        preview_rows = [
            {col: _fmt(val) for col, val in zip(cols, row.values())}
            for row in df.to_dict(orient="records")
        ]
        return ok({"columns": cols, "rows": preview_rows})
    except HTTPException:
        raise
    except Exception as exc:
        raise api_error("preview_error", f"Could not generate preview: {exc}", 500)
