...
def select_datasets(question: str, datasets: list) -> tuple[list[str], str | None, int, int]:
    """Return (selected_ids, raw_reasoning, tokens_input, tokens_output).
    ...
    tokens_input/output — token counts from the selector LLM call (0 if skipped).
    ...
    try:
        ...
        sel_ti, sel_to = response.tokens_input, response.tokens_output
    ...
    try:
        ...
        if isinstance(parsed, list) and parsed:
            ...
            if valid:
                ...
                return valid, raw, sel_ti, sel_to
        logger.warning("selector.empty_or_invalid", raw=raw[:200])
        return all_ids, raw, sel_ti, sel_to
